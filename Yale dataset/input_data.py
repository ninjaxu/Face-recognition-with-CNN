...
#D:\python\kewin_code\cats_vs_dogs\data
...
def get_batch(image, label, image_W, image_H, batch_size, capacity):
    ...
    image = tf.image.per_image_standardization(image)  #显示原图要标注掉此行。
...
#                                                      min_after_dequeue=CAPACITY-1)
    ...
    #image_batch = tf.cast(image_batch, tf.uint8)#图片失真就是类型问题，改成uint8形式，再去掉标准化就可以显示原图了。
                                                #标准化在110行，注释掉就好
